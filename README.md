...
- **专业工作台 UI**：左侧栏 + 主工作区，「暖陶工坊」视觉（奶油纸底 + 陶土橙强调 + 衬线标题，规格见 `docs/DESIGN_WEBUI.md`）、内联 SVG 图标，遵循 web-design skill 反“AI 味”规范
...
- **🗂 PowerPoint 联动 + 可视化编辑（新）**：导出的 pptx 在产物区点 **⇱** 一键用本机 PowerPoint 打开继续可视化编辑（走系统文件关联，零 COM）；编辑卡片支持**版式切换**（7 种版式，导出与 AI 设计共同遵循）与**每页缩略图预览**；本地做好的 pptx 拖回「导入 PPTX」即回工作台
- **🗑 删除 + 回收站（新）**：侧栏各列表悬停浮现 ✕，可删历史设计稿（联动清掉缩略图/动画/视频/同源导出文件）、历史项目记录、导出产物、pptx 导入记录；删除一律移入 `output/trash/` 保留 7 天可找回
- **开发友好**：Web 界面实时进度条与阶段、可折叠日志、风格徽章；1089 条 pytest 单测
